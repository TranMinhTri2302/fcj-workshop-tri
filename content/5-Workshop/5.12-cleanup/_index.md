...
**MUST cleanup after workshop to avoid charges!**

Even within Free Tier, some resources may incur charges:
- 💰 API Gateway requests (after 1M requests)
- 💰 DynamoDB On-Demand operations
- 💰 Rekognition calls
- 💰 S3 storage (after 5GB)
- 💰 CloudWatch Logs (after 5GB)
...
If script fails, cleanup manually in order:

**Step 1: WAF**
```bash
aws wafv2 list-web-acls --scope REGIONAL --region ap-southeast-1
# Disassociate and delete each Web ACL
```
...
4. Should be **$0.00** (or within Free Tier)
...
**Issue 1: S3 bucket won't delete**
```
...
**Issue 2: IAM role has attached policies**
```
...
**Issue 3: DynamoDB table in use**
```
Error: ResourceInUseException
```
**Solution:** Disable streams and PITR first
```bash
...
**1. Delete AWS Account** (optional, if only used for workshop)
- Go to Account Settings
- Close account
- Confirm via email

**2. Export CloudWatch Logs** (if need to keep for report)
```bash
...
**3. Take screenshots** (for internship report)
...
- [ ] Check AWS Billing = $0
- [ ] Export logs/screenshots for report
...
- API Gateway: $0.10
- DynamoDB: $2.50
- Rekognition: $300 (if traffic exists)
- S3: $0.50
- CloudWatch: $5.00
- **Total: ~$308/month**
...
**⚠️ IMPORTANT:** Setup billing alert IMMEDIATELY:
```bash
...
You have completed the **Smart Campus Serverless Attendance System Workshop**!
...
- ⭐ Apply for AWS certification exams!
