...
**Recommended order** (xóa theo thứ tự ngược lại với creation - dependency order):

1. **WAF WebACL associations** (phải disassociate trước khi delete)
2. **EventBridge Rules & Targets** (remove targets trước, delete rules sau)
3. **Lambda Functions & Event Source Mappings** (delete mappings trước)
4. **API Gateway** (HTTP API)
5. **SQS Queues (+ DLQ)** (purge messages trước)
6. **EventBridge Event Bus** (custom bus)
7. **Athena Workgroup** (recursive delete)
8. **Glue Crawlers, Tables, Database**
9. **S3 Buckets** (empty trước với --recursive, delete sau)
10. **Rekognition Collection**
11. **DynamoDB Tables** (tất cả 8 tables)
12. **CloudWatch Log Groups, Alarms, Dashboards**
13. **IAM Roles & Policies** (detach policies trước, delete role sau)
14. **Secrets Manager secrets** (force delete without recovery)
15. **SNS Topics**
16. **Kinesis Firehose Delivery Stream**
17. **Verify billing $0** (Billing console + Cost Explorer)
...
#### Manual Cleanup (Alternative - nếu script fails)

Nếu script fails, cleanup thủ công theo thứ tự dependency:
...
# Delete Web ACL (cần ID và LockToken)
...
# Delete event source mappings trước
...
**Expected:** Total cost = $0.00 (hoặc rất nhỏ < $0.01 cho API calls đã thực hiện trước cleanup)
...
| Issue | Cause | Fix |
|:---|:---|:---|
| `ResourceInUseException` DynamoDB | Table still deleting | Wait 1-2 min, retry delete |
| `DependencyViolation` IAM role | Policies still attached | Detach all policies first |
| `ConflictException` WAF | Lock token stale | Re-fetch LockToken, retry |
| `ResourceNotFoundException` | Already deleted | Ignore (idempotent) |
| S3 bucket not empty | Versioning enabled | `aws s3 rm s3://bucket --recursive` includes versions |
| Cognito delete fails | Deletion protection | Disable deletion protection first |
| Firehose delete fails | Destination still processing | Wait 5 min, retry |

---

#### Workshop Complete! 🎉

Bạn đã hoàn thành toàn bộ 12 bước của Smart Campus Workshop:

1. ✅ **Prerequisites** - AWS Account, IAM, CLI, Python
2. ✅ **DynamoDB** - 8 tables với GSI
...
10. ✅ **Security & Optimization** - IAM Least Privilege + WAF + Cost Opt
...
- Add more sophisticated ML (Bedrock for AI Assistant)

Chúc bạn thành công với các dự án AWS tiếp theo! ☁️🚀
