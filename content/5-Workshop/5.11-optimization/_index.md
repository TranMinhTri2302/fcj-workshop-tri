...
This is a **CRITICAL** step to ensure the Smart Campus Platform meets production-grade standards:
...
**Current issue:** Lambda roles use FullAccess policies (too broad, high security risk)

**Solution:** Create custom policies that grant only the permissions actually required for each Lambda function

**Policy for Main API Lambda (`smart-campus-api`):**
...
**Policy for Analytics Worker Lambda (`smart-campus-analytics-worker`):**
...
**Policy for Notification Worker Lambda (`smart-campus-notification-worker`):**
...
### 1.2. WAF (Web Application Firewall) - IP Whitelist for Attendance

**Purpose:** Allow check-in/check-out only from the company network (IP whitelist)

```bash
# 1. Create IP Set for company networks
...
# 2. Create Web ACL with IP whitelist rules
...
# 3. Associate WAF with API Gateway Stage
...
**Update the IP Whitelist from the Admin UI (Backend integration):**
```python
# app/modules/security/service.py
def update_waf_ip_set(new_ips: list[str]):
    """Admin calls this API to update the IP whitelist"""
...
```bash
# Enable when creating the table (or update existing)
...
```bash
# Test different memory sizes to find the optimal balance (cost vs duration)
...
# Enable Provisioned Concurrency to eliminate cold starts (production)
...
# Run to find the optimal memory size for cost/duration
```

### 2.2. API Gateway Caching

```bash
# Enable caching for GET endpoints (users list, reports)
...
```bash
# For predictable workloads, switch to Provisioned with Auto Scaling
aws dynamodb update-table \
...
**GSI Optimization:**
- Only project necessary attributes (KEYS_ONLY or INCLUDE specific attributes)
...
```sql
-- ALWAYS filter partition keys in the WHERE clause
...
### 4.1. Multi-AZ (Automatic for Serverless)
...
```bash
# CloudWatch Alarm for DLQ (already created in 5.10)
# Recovery procedure:
...
## Security & Optimization Summary Checklist
...
Proceed to [Step 12: Cleanup Resources](../5.12-cleanup) to remove all resources after the workshop!
      "OverrideAction": {
        "None": {}
      },
      "VisibilityConfig": {
...
**Recommendation:** Use **512MB** for the best balance of performance/cost
...
**Note:** On-Demand billing is simpler for the workshop!
...
**Speed improvement:** 50-500% faster uploads from remote locations
...
**Problem:** Rekognition is very expensive ($0.001/search × 10K/day = $300/month)
...
def recognize_face_cached(image_bytes):
    # Hash image to use as cache key
...
**Savings:** Eliminate 99% of liveness calls → $4,500 → $50/month
...
**Good news:** AWS services are Multi-AZ by default:
...
**Cost:** ~2x storage cost, but provides full DR capability
...
- [ ] Lambda timeout not set too high
