...
Trong bước này, bạn sẽ deploy 3 Lambda functions:
1. **Main API Lambda**: FastAPI backend chính (attendance, users, tasks...)
2. **Analytics Worker Lambda**: Xử lý events từ SQS Analytics Queue, ghi vào S3 Data Lake
3. **Notification Worker Lambda**: Xử lý events từ SQS Notification Queue, gửi email/SMS

#### Tại sao Lambda?

**Advantages:**
- ✅ Serverless: Không cần quản lý EC2 instances
- ✅ Auto-scaling: 0 → 10,000 concurrent tự động
- ✅ Pay-per-use: Chỉ trả tiền khi có request
- ✅ Built-in HA: Multi-AZ tự động
- ✅ Fast deployment: < 1 phút

**Limitations:**
- ❌ Cold start: 1-3 giây khi function idle
- ❌ Max timeout: 15 phút
- ❌ Max memory: 10GB
...
#### Bước 5: Test Lambda Locally (Optional)
...
#### Bước 6: Configure Lambda Triggers

**Add API Gateway trigger** (sẽ làm ở bước 6)
...
#### Bước 7: Enable CloudWatch Logs

Lambda tự động gửi logs đến CloudWatch. Verify:
...
#### Bước 8: Configure Environment Variables
...
Test với các memory sizes khác nhau:
```bash
# Test 256MB
aws lambda update-function-configuration \
  --function-name smart-campus-api \
...
**Rule of thumb:**
- 256MB: Chậm, rẻ (~$0.42/M invocations)
- 512MB: Balanced (~$0.83/M invocations)
- 1024MB: Nhanh, đắt (~$1.67/M invocations)

**Cold start optimization:**
- Use Python (cold start ~500ms) thay vì Java (~3s)
- Keep dependencies minimal
- Use Provisioned Concurrency (đắt hơn)
...
- **Duration**: Thời gian execution (aim: < 1000ms)
- **Errors**: Số lỗi (aim: < 1%)
- **Throttles**: Số lần bị throttle (aim: 0)
...
```
**Solution:** Tăng timeout hoặc optimize code

**Error: Out of memory**
```
Runtime exited with error: signal: killed Runtime.ExitError
```
**Solution:** Tăng memory từ 512MB → 1024MB

**Error: Import error**
```
Unable to import module 'app.handler': No module named 'fastapi'
```
**Solution:** Dependencies không được package đúng. Re-package với pip install -t

**Error: Permission denied**
```
User: arn:aws:... is not authorized to perform: dynamodb:PutItem
```
**Solution:** Check IAM role có đủ permissions
...
→ Workshop sẽ nằm trong Free Tier!

#### Verify Setup

Checklist:
- [ ] Lambda function `smart-campus-api` created và ACTIVE
...
Hãy chuyển sang [Bước 6: Configure API Gateway](../5.6-apigateway) để expose Lambda qua HTTP endpoint!
